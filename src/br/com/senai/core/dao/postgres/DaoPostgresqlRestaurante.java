...
public class DaoPostgresqlRestaurante implements DaoRestaurante {
	...
	private final String UPDATE = "UPDATE restaurantes SET " + "nome = ?, " + "descricao = ?, " + "cidade = ?, "
			+ "logradouro = ?, " + "bairro = ?, " + "complemento = ?, " + "id_categoria = ? " + "WHERE id = ?";
	...
	private final String SELECT_BY_NOME_CATEG = "SELECT " + "r.id id_restaurante, " + "r.nome nome_restaurante, "
			...
			+ "c.id id_categoria, c.nome nome_categoria " + "FROM categorias c, " + "restaurantes r "
			+ "WHERE r.id_categoria = c.id";

	private final String SELECT_TODES = "SELECT " + "r.id id_restaurante, " + "r.nome nome_restaurante, "
			+ "r.descricao, " + "r.cidade, " + "r.logradouro, " + "r.bairro, " + "r.complemento, "
			+ "c.id id_categoria, " + "c.nome nome_categoria " + "FROM restaurantes r, " + "categorias c "
			+ "WHERE r.id_categoria = c.id " + "ORDER BY LOWER(r.nome)";

	private final String SELECT_ID_EXISTENTE = "SELECT COUNT (restaurantes.id_categoria) as qtde "
			+ "FROM restaurantes " + "WHERE restaurantes.id_categoria = ?";
	...
	public void inserir(Restaurante restaurante) {
		...
		try {
			...
			throw new RuntimeException("Ocorreu um erro na inserção do restaurante." + " Motivo: " + ex.getMessage());
		} finally {
	...
	public void alterar(Restaurante restaurante) {
		...
		try {
			...
			throw new RuntimeException("Ocorreu um erro na alteração do " + "restaurante. Motivo: " + ex.getMessage());
		} finally {
			ManagerDb.getInstance().fechar(ps);
		}
	...
	public void excluirPor(int id) {
		...
		try {
			...
			throw new RuntimeException("Ocorreu um erro ao excluir o" + " restaurante. Motivo: " + ex.getMessage());
		} finally {
			ManagerDb.getInstance().fechar(ps);
		}
	...
	public List<Restaurante> listarTodas() {
		...
		try {
			...
			throw new RuntimeException("Ocorreu um erro na listagem" + " dos restaurantes. Motivo: " + ex.getMessage());
		} finally {
			ManagerDb.getInstance().fechar(ps);
			ManagerDb.getInstance().fechar(rs);
		}
	...
	private Restaurante extrairDo(ResultSet rs) {
		try {
			...
			throw new RuntimeException("Ocorreu um erro ao " + "extrair o restaurante. Motivo: " + ex.getMessage());
		}
	}
