	...
	public void validar(Restaurante restaurante) {
		if (restaurante != null) {
		    ...
		    if (isLogradouroInvalido) {
		        throw new IllegalArgumentException("O logradouro é obrigatório e deve conter"
		                + " até 200 caracteres.");
		    }
		...
		if (idRestaurante > 0) {
			...
			if (isRemocaoInvalida) {
				throw new IllegalArgumentException("Não é possível remover uma"
						+ " restaurante vinculada a um horário de atendimento.");
			}
	...
	public Restaurante buscarPor(int id) {
		if(id > 0) {
			...
			if(restauranteEncontrado == null) {
				throw new IllegalArgumentException("Não foi encontrada um restaurante com "
						+ "o código informado.");
			} 	
			return restauranteEncontrado;
		} else {
			throw new IllegalArgumentException("O id do restaurante deve "
					+ "ser maior que zero.");
		}
	}
