...
public class ViewGerenciaHorarioAtendimento extends JFrame {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					if (isAlteracao == false) {
						horarioAtendimento = new HorarioAtendimento(diasDaSemana, horaAbertura, horaFechamento,
								restaurante);
						...
						String mensagem = "O Restaurante " + horarioAtendimento.getRestaurante().getNome() + ""
								+ " teve seu h�rario de atendimento modificado. Novos hor�rios: ";
						...
						String mensagemSMS = mensagem + tabelaHorariosSms;
						String mensagemEmail = mensagem + tabelaHorariosEmail;
					...
					JOptionPane.showMessageDialog(contentPane, "Digite um valor para a hora v�lido.");
				} catch (Exception ex) {
					JOptionPane.showMessageDialog(contentPane, ex.getMessage());
				}
			}
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (!model.isVazio() && linhaSelecionada >= 0) {
					...
					if (horarioAtendimentoSelecionado != null) {
						...
						JOptionPane.showMessageDialog(contentPane, "Nenhum item selecionado para edi��o.");
					}
				} else {
					JOptionPane.showMessageDialog(contentPane, "Selecione uma linha para edi��o.");
				}
			}
		});
		...
		btnExcluir.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (linhaSelecionada >= 0 && !model.isVazio()) {
					int opcao = JOptionPane.showConfirmDialog(contentPane, "Deseja realmente excluir?", "Remo��o",
					...
					if (opcao == 0) {
						...
						try {
							...
							camposManter.add(cbRestaurantes);
							Utilitaria.limparCampos(contentPane, camposManter);
							JOptionPane.showMessageDialog(contentPane, "Hor�rio de atendimento exclu�do.");
						} catch (Exception ex) {
							JOptionPane.showMessageDialog(contentPane, ex.getMessage());
						}
					}
				} else {
					JOptionPane.showMessageDialog(contentPane, "Selecione uma linha para remo��o.");
				}
			}
		});
	...
	void atualizaTabelaHorarios(Restaurante restaurante) {
		...
		if (horarioEncontrado == null) {
			JOptionPane.showMessageDialog(contentPane,
					"N�o foi encontrado nenhum horario de atendimento " + "com os filtros informados.");
		}
	}
		...
		for (HorarioAtendimento horario : horarios) {
			...
			if (diaAnterior == null || !diaAnterior.equals(horario.getDiaSemana())) {
				...
				tabela.append("<td rowspan='" + rowspan + "' style='border: 1px solid black; text-align:left;'>")
						.append(horario.getDiaSemana()).append("</td>");
				diaAnterior = horario.getDiaSemana();
			}

			tabela.append("<td style='border: 1px solid black;'>").append(horario.getHoraAbertura().format(dtf))
					.append("</td>");
			tabela.append("<td style='border: 1px solid black;'>").append(horario.getHoraFechamento().format(dtf))
					.append("</td>");
			tabela.append("</tr>");
		}
	...
	public static String formatarTabelaHorariosSms(List<HorarioAtendimento> horarios) {
		StringBuilder tabela = new StringBuilder();
		DateTimeFormatter dtf = DateTimeFormatter.ofPattern("HH:mm");
		for (HorarioAtendimento horario : horarios) {
			tabela.append(horario.getDiaSemana()).append(" - ").append(horario.getHoraAbertura().format(dtf))
					.append(" �s ").append(horario.getHoraFechamento().format(dtf)).append("  |  ");
		}
		return tabela.toString();
	}
